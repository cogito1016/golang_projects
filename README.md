...
- 의도적으로 블록되길 원할 때 사용 
- 일반적으로
  - 닫힌채널에 메세지를 보내면 패닉을 일으킨다.
  - 닫힌채널에서 메세지를 읽으면 제로값을 받는다.
  - 즉, 채널을 닫은 뒤에는 쓸수는없지만 읽을수는 있다.
- nil 채널에서는
  - 읽거나 쓰려고 할 때 블록된다.
  - nil채널을 닫으려고 하면 패닉을 일으킨다.
  - ```go
     c := make(chan int)
     c = nul 
    ```
#### 2.14.7 버퍼채널
- 수행할 작업을 큐에 넣을 수 있다.
- 어플리케이션의 처리량을 제한하고자 버퍼채널을 세마포어처럼 사용할 수 있다.
- 채널을 만들 때 버퍼의크기를 지정하면 된다.
  - ```go 
    c := make(chan int, 4 )
    ```
